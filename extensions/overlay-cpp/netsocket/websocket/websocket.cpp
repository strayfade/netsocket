...
namespace {
    socket_t hostname_connect(const std::string& hostname, int port) {
        ...
        int ret = getaddrinfo(hostname.c_str(), sport, &hints, &result);
        Log("[socket] GetAddressInfo status: " + std::to_string(ret), "+");
        if (ret != 0) {
        ...
        for (p = result; p != NULL; p = p->ai_next) {
            ...
            int ConnectionStatus = connect(sockfd, p->ai_addr, p->ai_addrlen);
            Log("[socket] Connection status: " + std::to_string(ConnectionStatus), "+");
    ...
    easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
        ...
        if (origin.size() >= 200) {
            ...
            Log("[socket] Auto-detected host: " + std::string(host), "+");
            Log("[socket] Auto-detected port: " + std::to_string(port), "+");
        }
        else if (sscanf(url.c_str(), "ws://%[^:/]", host) == 1) {
            Log("[socket] Auto-detected host: " + std::string(host), "+");
            Log("[socket] Auto-detected port: N/A (80)", "+");
            port = 80;
        }
        else {
            Log("[socket] Unable to parse URL: " + url, "!");
            return NULL;
        }
        Log("[socket] Connecting to server \"" + url + "\"", "+");
        socket_t sockfd = hostname_connect(host, port);
        if (sockfd == INVALID_SOCKET) {
            Log("[socket] Unable to connect to server \"" + url + "\"!", "!");
            WSACleanup();
            return NULL;
        }
        char line[1024];
        int status;
        snprintf(line, 1024, "GET / HTTP/1.1\r\n");
        ...
        line[i] = 0;
        if (i == 1023) {
            Log("[socket] Invalid status line while connecting to server \"" + url + "\"!", "!");
            WSACleanup();
            return NULL;
        }
        sscanf(line, "HTTP/1.1 %d", &status);
        if (status != 101) {
            Log("[socket] Bad status (" + std::to_string(status) + ") while connecting to server \"" + url + "\"!", "!");
            Log("[socket] " + std::string(line), "!");
            WSACleanup();
            return NULL;
        }
        while (true) {
...
#endif
        Log("[socket] Connected to server \"" + url + "\"!", "+");
